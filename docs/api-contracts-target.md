...
- **POST /api/tickets**: Implementation validates title/description/priority/category, computes SLA due dates, ignores any `tags` field, writes an audit event, and returns 200 without idempotency headers.【F:src/app/api/tickets/route.ts†L9-L14】【F:src/app/api/tickets/route.ts†L52-L88】 Target requires `Idempotency-Key`, tag assignment, and may change status code to 201.
